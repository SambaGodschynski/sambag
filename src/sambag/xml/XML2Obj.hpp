...
private:
  ...
  typename BaseType::Ptr buildObject( const ticpp::Element &el ) {
    ...
    if ( it == tagMap.end() ) return typename BaseType::Ptr();
  ...
  struct IAttributeSetter {
    virtual void set ( typename BaseType::Ptr obj, const std::string &attrValue ) = 0;
  };
  //----------------------------------------------------------------------------
  template <typename AttrType, typename TagClassType, typename ObjectType>
  struct AttributeSetter : public IAttributeSetter {
    virtual void set ( typename BaseType::Ptr obj, const std::string &strValue ) {
  ...
  void setAttribute( typename BaseType::Ptr obj, const ticpp::Attribute &attr ) {
    using namespace boost::algorithm;
    typename AttributeMap::iterator it = attrMap.find( to_lower_copy( attr.Name() ) );
...
public:
  ...
  void registerAttribute( const std::string &attrName ) {
    IAttributeSetter *setter = new AttributeSetter<AttrType, TagClassType, ObjectType>();
