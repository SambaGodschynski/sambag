...
//-----------------------------------------------------------------------------
void ViewportLayout::addLayoutComponent(const AComponentSharedOrWeak &comp,
	com::ArbitraryType::Ptr constraint)
{
}
//-----------------------------------------------------------------------------
void ViewportLayout::layoutContainer(AContainerPtr parent) {
	Viewport::Ptr vp = std::dynamic_pointer_cast<Viewport>(parent);
	AComponentPtr view = vp->getView();
	if (!view) {
		return;
	}
	IScrollable::Ptr scrollable=std::dynamic_pointer_cast<IScrollable>(view);
